...
int main() {
    ...
    Shape *shapes2[] = {&circle1, &oval1, &circle2, &oval2, &circle3, &oval3}; //! Storing the address of the objects
    for (Shape* shape : shapes2) {
        std::cout << "Size of object inside the array: " << sizeof(*shape) <<std::endl;
        shape -> draw();
        std::cout << std::endl;
    }
